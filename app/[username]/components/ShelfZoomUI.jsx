...
const ShelfZoomUI = () => {
  ...
  const getShelfName = (index) => {
    const names = ['Bottom', 'Second', 'Third', 'Top'];
    return names[index] || `Shelf ${index + 1}`;
  };
  ...
  return (
    <>
      ...
      {isZoomed && (
        <Html>
          <div style={{
            ...
            📚 {getShelfName(selectedShelf)} Shelf (#{selectedShelf})
            <div style={{ 
              fontSize: '12px', 
              marginTop: '4px', 
              opacity: 0.9,
      ...
      {/* Instructions for shelf navigation - When not zoomed */}
      {!isZoomed && (
        <Html>
          <div style={{
            position: 'fixed',
      ...
      {/* Zoomed shelf controls - Bottom instructions when zoomed */}
      {isZoomed && (
        <Html>
          <div style={{
            position: 'fixed',
